...
export const BottomNav: React.FC<BottomNavProps> = ({ activeTab, onChangeTab }) => {
  ...
  return (
    <nav className="fixed bottom-3 sm:bottom-5 left-0 right-0 z-40 px-3 pointer-events-none no-print">
      <div className="max-w-md mx-auto pointer-events-auto">
        <div className="bg-slate-900/90 backdrop-blur-2xl text-white rounded-3xl p-1.5 shadow-2xl border border-white/10 flex items-center justify-around">
          {navItems.map((item) => {
            ...
            if (item.highlight) {
              return (
                <button
                  ...
                  className={`relative -top-4 flex flex-col items-center justify-center p-3 rounded-2xl shadow-xl transition-all active:scale-90 duration-200 group ${
                    isActive
            ...
            return (
              <button
                ...
                className={`relative flex flex-col items-center justify-center py-1.5 px-3 rounded-2xl transition-all duration-200 ${
                  isActive
                    ? 'text-white bg-white/10 font-bold scale-105'
                    : 'text-slate-400 hover:text-slate-200 font-medium'
                }`}
              >
                <Icon className={`w-5 h-5 transition-transform ${isActive ? 'scale-110 stroke-[2.3px] text-brand-400' : 'stroke-2'}`} />
                <span className="text-[10px] tracking-tight mt-0.5">{item.label}</span>
                {isActive && (
                  <span className="absolute bottom-1 w-1 h-1 rounded-full bg-brand-400 shadow-glow-brand" />
                )}
              </button>
            );
          })}
